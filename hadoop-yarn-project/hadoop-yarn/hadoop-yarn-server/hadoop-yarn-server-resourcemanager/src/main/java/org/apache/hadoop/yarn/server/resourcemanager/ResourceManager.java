  ...
  public static class SchedulerEventDispatcher extends AbstractService
    ...
    public void handle(SchedulerEvent event) {
      try {
        ...
        LOG.info("{intellij} " + event.getClass());
        if (event instanceof AppAddedSchedulerEvent) {
          LOG.info("{intellij} " + ((AppAddedSchedulerEvent) event).getApplicationId() + "put in eventQueue of rm.SchedulerEventDispatcher," +
                  "this evetn is SchedulerEventType.APP_ADDED");
        }
        LOG.info("{intellij} ScheduleraEvent put eventQueue of SchedulerEventDispatcher," +
                "then another thread of SchedulerEventDispatcher start take event from eventQueue," +
                "and scheduler start handle the event .");
