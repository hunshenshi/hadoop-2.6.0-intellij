...
public class RMAppManager implements EventHandler<RMAppManagerEvent>, 
  ...
  protected void submitApplication(
    ...
    LOG.info("{intellij} start create RMAppImpl object, just object");
    ...
    if (UserGroupInformation.isSecurityEnabled()) {
      ...
      // gets started.
      LOG.info("{intellij} start Dispatcher of rmContext, RMAPPEventType.START - RMAppEvent - ApplicationEventDispatcher");
      LOG.info("{intellij} 此处开始分开执行,application submit end, 等待rm对其状态进行处理");
      this.rmContext.getDispatcher().getEventHandler()
  ...
  private RMAppImpl createAndPopulateNewRMApp(
    ...
    LOG.info("{intellij} put RMAppImpl in rmApps of rmContext");
